...
// src/routes/spots.ts - Fix imports and add missing constant
import { Router, Request, Response, NextFunction } from 'express';
import { SpotService } from '../services/spotService';
import { authenticateToken, optionalAuth } from '../middleware/auth';
import { createError } from '../middleware/errorHandler';
import { SpotType, TransportMode, TRANSPORT_MODE_LABELS } from "../enum/enums"; // Add TRANSPORT_MODE_LABELS here
...
router.get('/for-mode/:mode', async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!Object.values(TransportMode).includes(mode as TransportMode)) {
            throw createError(`Invalid transport mode. Valid modes: ${Object.values(TransportMode).join(', ')}`, 400);
        }
        ...
        res.json({
            data: spots,
            mode,
            mode_label: TRANSPORT_MODE_LABELS[mode as TransportMode], // Now this will work
            criteria: {
                min_effectiveness: min_effectiveness || 4.0,
                location: latitude && longitude ? { latitude, longitude, radius } : null
            },
